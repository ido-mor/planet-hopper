...
When you bump cached assets, increment `CACHE_NAME` in `sw.js` (currently `planet-hopper-v12`) so installed copies pick up the change. Hard-refresh or re-open the Home Screen app after a cache bump.
