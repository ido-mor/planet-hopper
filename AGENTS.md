...
When you bump cached assets, increment `CACHE_NAME` in `sw.js` (currently `planet-hopper-v10`) so installed copies pick up the change. Hard-refresh or re-open the Home Screen app after a cache bump.
...
### Intro / launch
- After tap-to-play: the title letters fly apart sideways (`titleLetterFly`, per-letter spans built by `buildTitleLetters()`), the astronaut walks up the gantry (`astronautBoard`, 4.2s) and is occluded by the hull as they board, then countdown **3, 2, 1, GO!** from the `countdown.png` sprite sheet.
...
Keep existing timings unless asked: ship thrust 0.5s, feedback overlay 1.2s, title scatter 620ms + per-letter delay, countdown pop 1s, `runIntro()` walk beat (500ms delay, `ASTRONAUT_WALK_MS` 4200), walk cycle 0.72s over 12 frames. Flame flicker is a tiny loop. Do not add animation libraries.
